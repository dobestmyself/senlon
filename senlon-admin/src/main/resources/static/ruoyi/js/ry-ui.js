    ...
    $.extend({
    	...
    	table: {
			...
			exportDataExcel: function(){
            	...
            	var exportDataUrl = prefix + "/export";
            	$.post(exportDataUrl,$("#"+currentId).serializeArray(),function(result){
            		if(result.code === web_status.SUCCESS){
            			window.location.href = ctx + 'common/download?fileName=' + result.msg + '&delete='+true;
					}else{
            			$.modal.alertError(result.msg);
        ...
        operate: {
            ...
            detail: function(id, width, height) {
            	...
            	var _width = $.common.isEmpty(width) ? "2000" : width;
			...
			//TODO
			/**保存添加或修改信息*/
			addOrEditSaveTab: function(url,data){
				$.ajax({
					cache: true,
					type: "POST",
					url: url,
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.J_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;    //.J_iframe
    	            $.modal.close();
